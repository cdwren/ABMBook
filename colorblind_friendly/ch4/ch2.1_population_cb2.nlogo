...
to setup
  ca
  crt 500 [   ; 1000 originally
    set size 4   ; 5 originally
    set color (random 3 + 111) ; light or,pu = 26,116
  ]

  if walk = "IDD" [
    ask patches [
      set soil_quality 70  ; 100 originally
      ; this color setting doesn't do anything because it's handled below
      ; set pcolor scale-color blue soil_quality 0 100
      set pcolor palette:scale-gradient palette:scheme-colors
        "Sequential" "Greens" 5 soil_quality 0 100 ; PuBuGn
    ]
  ]
  ...
  ask n-of random 200 turtles [
    set color (random 3 + 111) ; dark or,pu = 21,111
  ]
  reset-ticks
end
...
to idd
  ...
  ask turtles [
    ...
    ;set pcolor scale-color blue soil_quality 0 100
    ; set pcolor scale-color yellow soil_quality 0 100
    set pcolor palette:scale-gradient palette:scheme-colors
      "Sequential" "Greens" 5 soil_quality 50 100  ; original BuGn
    ; try PuBuGn to see if that works better.
    ; Also try "Greens"
  ]
end
