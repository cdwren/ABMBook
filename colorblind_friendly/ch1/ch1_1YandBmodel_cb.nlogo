...
to setup
  clear-all
  import-pcolors "ch1.1_map.png"           ; map color isn't great, but it's ok
  create-turtles 20                        ; create 20 agents
  [                                        ; with the following state variables:
    set color 80 + random 19               ; colors between cyan and violet
    set size 2                             ; size
    set shape "turtle"                     ; turtle or circle shapes show up well
...
to reproduce
  ...
  if any? neighbors4 with [count turtles-here = 0 AND pcolor != white] [
    ...
    hatch 1 [
      ifelse color < 120 [                  ; color must stay below magenta
        set color color + 0.1               ; new turtles are clones of parents with small shift in color
      ]
      [
        set color 80                        ; if color has gone into magenta, make cyan again
      ]
