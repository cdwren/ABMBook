...
to go
  ...
  if ticks >= 500 [ stop ]
  if count turtles >= 500 or count turtles = 0 [ stop ]

  ;; Two moves per year
  repeat 2 [move-season]
...
to move-season ;; ger procedure
  ...
  ask turtles[
    set energy energy - energy-use      ;; each season require energy
...
to move-local    ;; ger procedure
  ;; ger moves to a productive patch if available in the closest surrounding
  if pcolor != green [
      if any? neighbors with [ pcolor = green ][
      set energy energy - energy-use * 0.5
...
to graze ;; ger procedure
  ;; ger eats grass, turns the patch brown
  if pcolor = green [
    set pcolor brown
    set energy energy + gain-from-food  ;; ger gain energy by eating
  ]
end


to update-history  ;; ger procedure
  ;; remember the current patch as productive
  if pcolor = green [
...
to variability ;; patch procedure
  ; depending on the environmental variability some patches will become barren
  if pcolor = green  [
...
to reproduce-gers  ;; ger procedure
  ;; if enough energy available and the probability of reproduction is met create a new ger
  if energy >= 20 [
 ...
 to death  ;; ger procedure
 ;  when energy dips below five, die
  if energy < 5 [ die ]
...
This is example model used in chapter 2.1 of Romanowska, I., Wren, C., Crabtree, S. 2021 Agent-based modelling for archaeologists. Santa Fe Institute Press. 
