...
patches-own [vegetation_type]
hunters-own [done my_camp time-left]

to setup
  ca
  ask patches [
    set vegetation_type pxcor
  ...
  set time-walk-cell 0.1

end

to forage     ;runs once per tick, i.e. the go procedure
  ;...
  ask camps [   ;camps are defined as a new agent breed and represent a "group"
                ; random-walk version
    set stepsize 20
    let found 0
    while [found = 0] ; test that the new camp will be within the map
    [
      lt random 360
      if patch-ahead stepsize != nobody [; 20 is 2 km.
        if [vegetation_type] of patch-ahead stepsize > 0 [ ;i.e. part of the map
          move-to patch-ahead stepsize
          set found 1
        ]
      ]
    ]
  ...
  ;...
  ;Hunter mobility section
  ask hunters [set time-left 12 set done 0]
  let allhuntersdone 0 ; this defines when all hunters are back in the camp (allhuntersdone  = 1)
  while [allhuntersdone = 0]
  [
    ask hunters with [done = 0][
      let walk-time-to-camp (distance my_camp * time-walk-cell)
      ifelse time-left > walk-time-to-camp [
        ;move & hunt procedure which moves loosely in the direction of camp
        ;...
        lt random 360
        ...
        if time-left <= 0 OR distance my_camp < 1 [
          move-to my_camp
          set done 1
        ]
      ]
