...
to memory-walk
  ask turtles [
    ;;Move towards remembered targets first, if none available then random walk
    ifelse any? memory with [pcolor = white] [
      let p one-of memory with [pcolor = white]
      face p
      fd 1
    ][ ;else no patches in memory are currently available
      set heading random 360
      fd 1
    ]

    ;add the new patch to memory then highlight remembered area with the agent’s color
    set memory (patch-set memory patch-here)
    ask memory with [pcolor != white and pcolor != grey] [set pcolor [color] of myself - 2]
