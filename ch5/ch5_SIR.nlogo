...
Agents move at random. Among them a small proportion is infected and can transmit the disease/cultural trait onto their susceptible neighbours. 
After 9 time steps agents recover and cannot infect any more. Recovered agents cannot be reinfected.
...
- experiment with the length of infection. Make a slider and investigate how short the infection would have to be for the population to not get overwhelmed by the disease.
- see what happens when the mobility is reduced, e.g., only 10% of agents can move. 
- change the code so that turtles that recover become susceptible after a short while.


## RELATED MODELS

There is a large number of more complex SIR models, including in the NetLogo library, e.g., "epiDEM Basic", "Spread of Disease", or "Virus on a Network" models.
